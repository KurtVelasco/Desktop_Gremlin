...
namespace Desktop_Gremlin
{
    public partial class SpriteSheet : Window
    {
        ...
        private Bitmap WALK_DOWN_SHEET;
        private Bitmap IDLE_SHEET;
        private Bitmap GRAB_SHEET;
        private Bitmap WALK_UP_SHEET;  
        private Bitmap WALK_LEFT_SHEET;
        private Bitmap WALK_RIGHT_SHEET;   
        private Bitmap EMOTE1_SHEET;
        private Bitmap EMOTE2_SHEET;
        private Bitmap EMOTE3_SHEET;   
        ...
        public SpriteSheet()
        {
            ...
            App.hook = new TaskPoolGlobalHook();
            App.hook.MouseMoved += (sender, args) => MouseMoved(sender, args);
            App.hook.MouseReleased += (sender, args) => MouseReleased(sender, args);
            App.hook.RunAsync();
            SetupTrayIcon();
            LoadConfig();
            LoadSpritesSheet();
        ...
        private Bitmap LoadSprite(string fileName)
        {
            string path = System.IO.Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "Sprites", "SpriteSheet", fileName);

            if (!File.Exists(path))
            {
                throw new FileNotFoundException($"Missing sprite: {fileName} at {path}");
            }
        ...
        private int PlayAnimationFrame(Bitmap sheet, int currentFrame, int frameCount, int columns = 5)
        {
            if (sheet == null) return currentFrame;

            int x = (currentFrame % columns) * FRAME_WIDTH;
            int y = (currentFrame / columns) * FRAME_HEIGHT;

            if (x + FRAME_WIDTH > sheet.PixelSize.Width || y + FRAME_HEIGHT > sheet.PixelSize.Height)
                return currentFrame;

            SpriteImage.Source = new CroppedBitmap(sheet, new PixelRect(x, y, FRAME_WIDTH, FRAME_HEIGHT));
       ...
       { 
           ...
           MASTER_TIMER = new DispatcherTimer { Interval = TimeSpan.FromMilliseconds(intervalMs) };
           MASTER_TIMER.Tick += (s, e) =>
            {
                ...
                if (FOLLOW_CURSOR && !IS_DRAGGING)
                {
                    ...
                    double targetX = MousePosition.X - (FRAME_WIDTH / 2);
                    double targetY = MousePosition.Y - (FRAME_HEIGHT / 2);
                ...
                else if (IS_WALKING)
                {
                    ...
                    this.Position = new PixelPoint(
                        this.Position.X + (int)MOUSE_DELTAX,
                        this.Position.Y + (int)MOUSE_DELTAY
                    );
       ...
       //This uses the already necessary SharpHook package to double check that it happens. Yeah, I know.
       ...
       void MouseMoved(object? sender, MouseHookEventArgs args )
       {
           MousePosition = new Vector2(args.Data.X, args.Data.Y);
       } 
        ...
        private void LoadConfig()
        {
            ...
            if (!File.Exists(path))
            {
                throw new FileNotFoundException("Where the Fuu is the config file!?", path);
            }
        ...
        private void SetupTrayIcon()
        {
            TRAY_ICON = new TrayIcon
            {
                ...
                ToolTipText = "Desktop Gremlin"
            };

            var menu = new NativeMenu();

            var reappearItem = new NativeMenuItem("Reappear");
            reappearItem.Click += (s, e) => ResetApp();

            var closeItem = new NativeMenuItem("Explode (Close)");
            closeItem.Click += (s, e) => CloseApp();

            menu.Items.Add(reappearItem);
            menu.Items.Add(closeItem);

            TRAY_ICON.Menu = menu;
            TRAY_ICON.IsVisible = true;
        }
